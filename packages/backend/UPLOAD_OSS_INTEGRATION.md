...
async function getOSSConfig() {
  ...
  const ossConfig = {
    ...
    accessKeyId: storageConfigs.find(c => c.key === 'oss_access_key_id')?.value,
    accessKeySecret: storageConfigs.find(c => c.key === 'oss_access_key_secret')?.value,
    endpoint: storageConfigs.find(c => c.key === 'oss_endpoint')?.value,
  };

  // 验证配置完整性
  if (ossConfig.region && ossConfig.bucket &&
...
2. **上传图片**
   - 图片自动上传到阿里云 OSS
   - 返回 OSS URL：`https://my-bucket.cn-hangzhou.aliyuncs.com/homepage/backgrounds/xxx.jpg`
