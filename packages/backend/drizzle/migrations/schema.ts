...
export const homepageSections = pgTable(
  ...
  (table) => [
    index('homepage_sections_order_idx').using(
      'btree',
 ...
 */
export const userWechatBindings = pgTable(
  ...
  (table) => [
    ...
    index('user_wechat_bindings_user_id_idx').using(
      'btree',
      table.userId.asc().nullsLast().op('text_ops')
    ),
    index('user_wechat_bindings_unionid_idx').using(
      'btree',
      table.unionid.asc().nullsLast().op('text_ops')
    ),
  ]
);
...
export const paymentOrders = pgTable(
  ...
  (table) => [
    index('payment_orders_user_id_idx').using(
      'btree',
      table.userId.asc().nullsLast().op('text_ops')
    ),
    index('payment_orders_status_idx').using('btree', table.status.asc().nullsLast()),
    index('payment_orders_transaction_id_idx').using(
      'btree',
...
export const paymentLogs = pgTable(
  ...
  (table) => [
    index('payment_logs_order_id_idx').using(
      'btree',
