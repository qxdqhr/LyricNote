...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!section) {
      return NextResponse.json({ success: false, error: '首页配置不存在' }, { status: 404 });
    }
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ success: false, error: '更新首页配置失败' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    await db.delete(homepageSections).where(eq(homepageSections.id, parseInt(params.id)));

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('删除首页配置失败:', error);
    return NextResponse.json({ success: false, error: '删除首页配置失败' }, { status: 500 });
  }
}
