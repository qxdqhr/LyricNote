...
class ApiService {
  ...
  async init() {
    try {
      this.token = Taro.getStorageSync(AUTH_TOKEN_KEY)
    } catch (error) {
  ...
  async saveUserData(user: User) {
    Taro.setStorageSync(USER_DATA_KEY, JSON.stringify(user))
  }

  // 获取用户数据
  async getUserData(): Promise<User | null> {
    try {
      const data = Taro.getStorageSync(USER_DATA_KEY)
  ...
  // 清除用户数据
  async clearUserData() {
    Taro.removeStorageSync(USER_DATA_KEY)
    Taro.removeStorageSync(AUTH_TOKEN_KEY)
    this.token = null
  }
