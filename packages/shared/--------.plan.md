...
```typescript
- order_id
- action (create/notify/query/refund)
...
```typescript
// 签名验证
export function verifyWechatSignature(data: any, signature: string, key: string): boolean
...
class WechatAuthService {
  ...
  // 绑定或创建用户
  async bindOrCreateUser(platform: string, userInfo: WechatUserInfo): Promise<User>
}
```
...
class WechatPaymentService {
  ...
  // 退款
  async refund(orderId: string, refundAmount: number, reason: string): Promise<any>
